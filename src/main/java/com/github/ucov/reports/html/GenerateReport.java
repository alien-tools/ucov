...
public class GenerateReport {
    ...
    public void generateReport() {
        ...
        for (String packageName : packages) {
            dossier = new File(reportFolder + "/" + packageName);
            dossier.mkdir();
    ...
    private void generatePackageIndex(String packageName) {
        ...
        try {
            FileWriter fw = new FileWriter(reportFolder + "/" + packageName + "/index.html");
            ...
            for (CompatibilityElement compatibility : compatibilities) {
                ...
                pw.println("<tr>");
                pw.println("<td class='link'><a href=" + compatibility.fileName() + ".html>" + compatibility.fileName() + "</a></td>");
            ...
            Main.UCOV_LOGGER.info("Message " + e);
        }
    }
    ...
    private void generateGeneralIndex() {
        ...
        try {
            ...
            FileWriter fw = new FileWriter(reportFolder + "/index.html");
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter pw = new PrintWriter(bw);
            ...
            for (CompatibilityElement compatibility : compatibilities) {
                ...
                pw.println("<tr>");
                pw.println("<td class='link'><a href=./" + compatibility.packageName() + "/index.html>" + compatibility.packageName() + "</a></td>");
                pw.println(getTdStringForOneELement(i, percentage.get("Tested"), compatibility.nbFieldTested() + compatibility.nbMethodTested() + compatibility.nbTypeTested() + compatibility.nbConstructorTested(), percentage.get("HalfTested"), compatibility.nbFieldHalfTested() + compatibility.nbMethodHalfTested() + compatibility.nbTypeHalfTested() + compatibility.nbConstructorHalfTested(), percentage.get("NoTested"), compatibility.nbFieldNoTested() + compatibility.nbMethodNoTested() + compatibility.nbTypeNoTested() + compatibility.nbConstructorNoTested()));
                i++;
                pw.println(getTdStringForOneELement(i, percentage.get("Field_Tested"), compatibility.nbFieldTested(), percentage.get("Field_HalfTested"), compatibility.nbFieldHalfTested(), percentage.get("Field_NoTested"), compatibility.nbFieldNoTested()));
                i++;
    ...
    private void generateFile(String packageName, String fileName, String fileSource) {
        try {
            ...
            FileWriter fw = new FileWriter(reportFolder + "/" + packageName + "/fileName.html");
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter pw = new PrintWriter(bw);
            ...
            for (ApiElement element : elements) {
                ...
                pw.println("<tr>");
                pw.println("<td class='column'>" + element.symbolType.name() + "</td>");
                pw.println("<td class='column'>" + element.name + "</td>");
    ...
    private void generateCss() {
        try {
            ...
            FileWriter fw = new FileWriter(reportFolder + "/report.css");
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter pw = new PrintWriter(bw);
    ...
    private void generateJS() {
        try {
            ...
            FileWriter fw = new FileWriter(reportFolder + "/report.js");
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter pw = new PrintWriter(bw);
    ...
    private String getTdStringForOneELement(int i, int percentageTested, int nbTested, int percentageHalfTested, int nbHalfTested, int percentageNoTested, int nbNoTested) {
        int height = 6;
        return "<td class='column'><button type='button' class='bouton' data-bs-toggle='tooltip' data-bs-html='true' data-bs-title='<div>Tested : " + percentageTested + "% | " + nbTested + "</div><div>Half Tested : " + percentageHalfTested + "% | " + nbHalfTested + "</div><div>No Tested : " + percentageNoTested + "% | " + nbNoTested + "</div>'><canvas id = 'TestedCanvas_" + i + "' width=" + percentageTested + " height=" + height + "></canvas><canvas id = 'HalfTestedCanvas_" + i + "' width=" + percentageHalfTested + " height=" + height + "></canvas><canvas id = 'NoTestedCanvas_" + i + "' width=" + percentageNoTested + " height=" + height + "></canvas></button></td>";
    }
    ...
    private String getStringForOneElement(CompatibilityStatus compatibilityStatus, int i, ApiElement element, ClientType clientType) {
        int height = 20;
        int length = 20;
        String result = "<td class='column'><button type='button' class='bouton' data-bs-toggle='tooltip' data-bs-html='true' data-bs-title='" + getTooltip(element, clientType) + "'>";
        switch (compatibilityStatus) {
            case TESTED ->
                    result += "<canvas id = 'TestedCanvasCircle_" + i + "' width=" + length + " height=" + height + "></canvas>";
            case HALF_TESTED ->
                    result += "<canvas id = 'HalfTestedCanvasCircle_" + i + "' width=" + length + " height=" + height + "></canvas>";
            case NO_TESTED ->
                    result += "<canvas id = 'NoTestedCanvasCircle_" + i + "' width=" + length + " height=" + height + "></canvas>";
        }
        result += "</button></td>";
    ...
    private String getTooltip(ApiElement element, ClientType clientType) {
        ...
        if (element.symbolType == SymbolType.FIELD) {
            result += "<div> Field Read : " + database.countNbUsage(element.elementId, UsageType.FIELD_READ.name(), apiType, clientType) + "</div>";
            if (!(element.modifier == Modifier.FINAL)) {
                result += "<div> Field Write : " + database.countNbUsage(element.elementId, UsageType.FIELD_WRITE.name(), apiType, clientType) + "</div>";
            }
        } else if (element.symbolType == SymbolType.CLASS) {
            if (!(element.modifier == Modifier.FINAL)) {
                result += "<div> Type Extend : " + database.countNbUsage(element.elementId, UsageType.TYPE_EXTEND.name(), apiType, clientType) + "</div>";
            }
            result += "<div> Type Reference : " + database.countNbUsage(element.elementId, UsageType.TYPE_REFERENCE.name(), apiType, clientType) + "</div>";
        } else if (element.symbolType == SymbolType.INTERFACE) {
            result += "<div> Type Implement : " + database.countNbUsage(element.elementId, UsageType.TYPE_IMPLEMENT.name(), apiType, clientType) + "</div>";
            result += "<div> Type Reference : " + database.countNbUsage(element.elementId, UsageType.TYPE_REFERENCE.name(), apiType, clientType) + "</div>";
        } else if (element.symbolType == SymbolType.ENUM) {
            result += "<div> Type Reference : " + database.countNbUsage(element.elementId, UsageType.TYPE_REFERENCE.name(), apiType, clientType) + "</div>";
        } else if (element.symbolType == SymbolType.METHOD) {
            if (!(element.modifier == Modifier.FINAL)) {
                result += "<div> Method Override : " + database.countNbUsage(element.elementId, UsageType.METHOD_OVERRIDE.name(), apiType, clientType) + "</div>";
            }
            if (!(element.modifier == Modifier.ABSTRACT)) {
                result += "<div> Method Call : " + database.countNbUsage(element.elementId, UsageType.METHOD_CALL.name(), apiType, clientType) + "</div>";
            }
        } else {
            result += "<div> Constructor Call : " + database.countNbUsage(element.elementId, UsageType.CONSTRUCTOR_CALL.name(), apiType, clientType) + "</div>";
        }
        return result;
    }
}
